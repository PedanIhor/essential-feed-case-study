...
//  EssentialFeedTests
//
//  Created by Ihor Pedan on 24/02/2026.
//
...
final class CodableFeedStoreTests: XCTestCase {
    ...
    func test_retrieve_hasNoSideEffectsOnDeliverFailure() {
        let storeURL = self.testSpecificStoreURL()
        let sut = makeSUT(storeURL: storeURL)
    ...
    func test_delete_deliversErrorOnFailedDeletion() {
        ...
        XCTAssertNotNil(deletionError, "Expected failed cache deletion to fail with an error")
        expect(sut, toRetrieve: .empty)
    }
    ...
    func test_storeSideEffects_runSerially() {
        ...
        XCTAssertEqual([op1, op2, op3], completedOperationsInOrder, "Expected side-effects to run serially but operation completion order was different")
    }
    ...
    private func expect(_ sut: FeedStore, toRetrieve expectedResult: RetrieveCachedFeedResult, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.retrieve { retrievedResult in
            ...
            case let (.found(expectedFeed, expectedTimestamp), .found(retrievedFeed, retrievedTimestamp)):
                XCTAssertEqual(expectedFeed, retrievedFeed, "Expected cached feed to match the one retrieved", file: file, line: line)
                XCTAssertEqual(expectedTimestamp, retrievedTimestamp, "Expected cached timestamp to match the one retrieved", file: file, line: line)
